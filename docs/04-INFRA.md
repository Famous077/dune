...
**Deployed this weekend:** `IndexStateMachine` and the four pipeline functions were dropped for one `IndexFn` (3 GB memory, 15-minute timeout, 2 GB ephemeral storage) that runs the whole pipeline in a single invocation, started asynchronously by `ApiFn`. The reasons and trade-offs are in `01-BACKEND.md`, "Indexing pipeline". The sizing notes below apply to `IndexFn` as they would have to `CloneFn`. `McpService` is likewise replaced, by `McpFn`, a Lambda on the same HTTP API, because App Runner is not available to our account; see "MCP service deployment" below.
...
**Deployed this weekend:** App Runner is not available to our account (`SubscriptionRequiredException` in every region), so the MCP server is `McpFn`, a Lambda in the same SAM template, on the existing HTTP API at `/mcp` (the `McpUrl` stack output). It deploys with `npm run deploy` like everything else. Streamable HTTP only; see `01-BACKEND.md`, "MCP server".
